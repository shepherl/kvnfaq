...
install_app() {
	...
	if [[ $(hostname) == *.kzn.21-school.ru ]]; then
		...
		if [ -z "$APP_URL" ]; then
			osascript -e 'display dialog "Could not find the installation file on GitHub." buttons {"OK"} default
  button 1 with title "Error" with icon stop'
			exit 1
		fi
		...
		osascript -e 'display dialog "KVN installation completed successfully! The app is on your desktop."
  buttons {"OK"} default button 1 with title "Installer" with icon note'
	else
		osascript -e 'display dialog "Your device is not eligible for installation." buttons {"OK"} default
  button 1 with title "Error" with icon caution'
	fi
	exit 0
}
...
if ! check_space; then
  ...
  temporary files? This will not affect your personal files." buttons {"Cancel", "Clean"} default button 2 with title
  "Storage Error" with icon caution')
  ...
  delete unnecessary files and try again." buttons {"OK"} default button 1 with title "Error" with icon stop'
			exit 1
		fi
	else
		exit 1
